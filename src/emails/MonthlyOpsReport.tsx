...
import type { MonthlyReportData } from '@/lib/reports/monthly'
import {
  formatCurrency,
  formatEnglishDate,
  formatNumber,
...
/**
 * The monthly ops report — same structure as the weekly digest, one level
 * zoomed out. Covers the most recently completed calendar month.
 *
 * Not yet wired to a cron schedule — content-only for now, rendered via
 * the admin preview page while it's being finalized.
 */
export default function MonthlyOpsReport({ data, browserUrl }: Props) {
  const {
    monthStart,
    monthEnd,
    revenue,
  ...
  const worstWeek = [...weeks].sort((a, b) => a.revenue - b.revenue)[0]
  ...
  const previewText = `Month ${monthStart} → ${monthEnd} · Sales ${formatCurrency(revenue.totalSales)} · Collected ${formatCurrency(revenue.revenueCollected)} · ${revenue.deliveredCount} delivered · ${customers.newCustomers} new`
        ...
        <Container style={{ backgroundColor: '#ffffff', maxWidth: 720, margin: '0 auto', border: `1px solid ${colors.border}`, borderRadius: 8, overflow: 'hidden' }}>
          ...
          <Section style={{ backgroundColor: colors.brand, padding: '20px 24px' }}>
            <Heading as="h1" style={{ color: '#fff', margin: 0, fontSize: 20, fontWeight: 700 }}>
              🗓️ Monthly Ops Report
            </Heading>
            <Text style={{ color: '#fecaca', margin: '4px 0 0', fontSize: 13 }}>
              {formatEnglishDate(monthStart)} — {formatEnglishDate(monthEnd)}
            </Text>
          </Section>

          {/* Section 1 — KPI strip */}
          <Section style={sectionStyle}>
            <SectionHeading emoji="📊" label="Month in a Glance" />
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  <th style={thStyle}>Metric</th>
                  <th style={thStyle}>This Month</th>
                  <th style={thStyle}>vs Last Month</th>
                </tr>
              </thead>
              <tbody>
                <KpiRow label="Sales (orders placed)" value={formatCurrency(revenue.totalSales)} change={revenue.salesPctChange} bold />
            ...
            <Text style={{ ...subtleNote, marginTop: 8 }}>
              “Sales” = value of all orders placed regardless of status; “Revenue Collected” = net amount actually collected on delivered orders.
            </Text>
          </Section>
          ...
          <Section style={sectionStyle}>
            ...
            {revenueByOrderType.length === 0 ? (
              <Text style={subtleNote}>No orders this month.</Text>
            ) : (
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
          ...
          <Section style={sectionStyle}>
            <SectionHeading emoji="📅" label="Week by Week" />
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
              ...
              <tbody>
                {weeks.map((w, i) => (
                  <tr key={w.weekStart} style={i % 2 === 1 ? { backgroundColor: colors.bg } : undefined}>
                    <td style={{ ...tdStyle, textAlign: 'left', fontWeight: 600 }}>
                      {w.label} <span style={{ fontWeight: 400, color: colors.subtle }}>({w.weekStart} → {w.weekEnd})</span>
                    </td>
                    <td style={tdStyle}>{formatNumber(w.orders)}</td>
                    <td style={{ ...tdStyle, color: colors.positive }}>{formatNumber(w.delivered)}</td>
                    <td style={{ ...tdStyle, fontWeight: 600 }}>{formatCurrency(w.revenue)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            {bestWeek && worstWeek && bestWeek.weekStart !== worstWeek.weekStart && (
              <Text style={{ ...subtleNote, marginTop: 8 }}>
                Best week: <b>{bestWeek.label}</b> ({formatCurrency(bestWeek.revenue)}) · Worst week: <b>{worstWeek.label}</b> ({formatCurrency(worstWeek.revenue)})
              </Text>
            )}
          </Section>
          ...
          <Section style={sectionStyle}>
            ...
            {topProducts.length === 0 ? (
              <Text style={subtleNote}>No delivered orders this month.</Text>
            ) : (
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
          ...
          <Section style={sectionStyle}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 12 }}>
              <tbody>
                <MiniRow label="Opened this month" value={formatNumber(complaints.opened)} extra={complaints.openedPctChange.text} extraColor={changeColor(complaints.openedPctChange.sign, true)} />
                <MiniRow label="Closed this month" value={formatNumber(complaints.closed)} />
          ...
          <Section style={sectionStyle}>
            ...
            {staff.length === 0 ? (
              <Text style={subtleNote}>No activity this month.</Text>
            ) : (
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
          ...
          <Section style={sectionStyle}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 8 }}>
              <tbody>
                ...
                <MiniRow label="Active products with zero sales this month" value={formatNumber(inventory.zeroSalesTotal)} />
              </tbody>
            </table>
