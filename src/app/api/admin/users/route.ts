...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[api/admin/users GET] unhandled:', err)
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Upsert so this works whether or not the on_auth_user_created trigger
    // fired first. Guarantees a profile row exists with the admin-supplied
    // name/role.
    ...
    console.error('[api/admin/users POST] unhandled:', err)
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'Internal error' },
      { status: 500 },
    )
